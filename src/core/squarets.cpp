#include "squarets/core/squarets.hpp" // IWYU pragma: associated
...
static const std::string kOutVarName = "squarets_output";
...
{
#if !defined(NDEBUG)
  BOOST_OUTCOME_TRY(performDebugChecks(supported_tags()));
#endif
...
#if !defined(NDEBUG)
  dbgOut += base::ASCIIToUTF16("___[[kOpenTagStart]]___");
...
outcome::result<void, GeneratorErrorExtraInfo> Generator::handleTag(
  ...
  outcome::result<
    void,
    squarets::core::errors::GeneratorErrorExtraInfo> cb_result
      = outcome::success();
  ...
  for(size_t i = 0; i < tag.close_tag.opening.size(); i++) {
    ...
    Generator::EncloseTagResult closedTag =
      BOOST_OUTCOME_TRYX(closedTagResult);
